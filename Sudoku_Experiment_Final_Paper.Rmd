...
```{r models-outcome-cace, echo=FALSE, tab.cap="ITT and CACE Estimates for Puzzle Completion and Solve Time. Time analysis conditional on solvers only (N=28). No estimates are statistically significant.", results='asis'}
...
Combined, these results appear to indicate that participants who spent more time on the problem solved it more often. However, the correlation of time and completion is $\rho$ = `r cor_time_completion_r` (p = `r cor_time_completion_p`) showing that those outcomes are actually not closely tied together.

The only significant finding was that participants who had previously played sudoku performed `r abs(sudoku_t_adj_coef)` minutes better than participants who were new to the game. 
