...
The randomization did not produce a uniform distribution across all groups. The control group had `r n_control` compared to `r n_video` assigned to video and `r n_ai` assigned to AI Tutor. With total participation of `r n_total` participants, the data distribution is unbalanced with `r n_control_pct`% assigned to control, `r n_video_pct`%  to video, and `r n_ai_pct`% to AI tutor.
...
The participants were divided into two treatments and one control group.  The participant solved a Killer Sudoku puzzle (Figure 1), which is a variant of Sudoku with some additional rules - namely the puzzle board features boxes that cover multiple cells with a number that is the sum of all the digits in the box. The treatment offered participants 3 different ways to learn the rules of the puzzle before they were tested on their understanding of the puzzle rules and strategy.
